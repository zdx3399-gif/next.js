...
const REASON_LABEL: Record<string, string> = {
  ...
  "No stable historical answer available": "找不到穩定且可參考的歷史答案",
  "Low-risk issue with high confidence candidate": "低風險問題且候選答案信心高",
  "Confidence below auto-apply threshold": "信心分數未達自動修正門檻",
  "Issue type is not in low-risk list": "問題類型不在低風險清單中",
}
...
export function AiAutoFixPanel({ readOnly = false }: { readOnly?: boolean }) {
  ...
  const issueOptions = useMemo(() => {
    const values = Array.from(new Set(items.map((x) => x.issueType).filter(Boolean))).sort()
    return values
  }, [items])
  ...
  const filteredItems = useMemo(() => {
    ...
    return items.filter((item) => {
      ...
      if (keyword) {
        const hay = `${item.clusterKey || ""} ${item.aiOriginalAnswer || ""} ${item.proposedAnswer || ""} ${item.crowdFeedbackSummary || ""}`.toLowerCase()
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const message = e instanceof Error ? e.message : "讀取資料失敗"
      setError(message)
    } finally {
      setLoading(false)
    }
    ...
    <div className="space-y-4">
      <div className="rounded-2xl border border-[var(--theme-border)] bg-[var(--theme-bg-primary)] p-4 sm:p-5">
        ...
        <div className="mt-4 grid grid-cols-2 gap-3 lg:grid-cols-5">
          ...
          <KpiCard label="問題類群總數" value={summary?.totalClusters} />
          <KpiCard label="需要處理的類群" value={summary?.triggeredClusters} />
          <KpiCard label="可直接修正" value={summary?.autoApplyCount} />
          <KpiCard label="先人工確認" value={summary?.manualReviewCount} />
        </div>
        ...
        <div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-4">
          ...
          <Field label="關鍵字（問題/答案）">
            <input
              type="text"
              value={keywordFilter}
              onChange={(e) => setKeywordFilter(e.target.value)}
              placeholder="例如：公告、報修"
              className="w-full rounded-md border border-[var(--theme-border)] bg-[var(--theme-bg-card)] px-2 py-2 text-sm text-[var(--theme-text-primary)]"
            />
          </Field>
        </div>
            ...
            return (
              <div key={`${item.clusterKey}-${item.issueType}-${confidenceText}`} className="rounded-xl border border-[var(--theme-border)] bg-[var(--theme-bg-primary)] p-4">
                ...
                <p className="mt-1 text-xs text-[var(--theme-text-secondary)]">
                  AI 補跑狀態：{statusLabel(item.aiRerunStatus)}（來源：{item.aiRerunSource === "cache" ? "快取" : "即時"}）
                  {item.aiRerunError ? `（${item.aiRerunError}）` : ""}
                </p>
                ...
                <div className="mt-3 grid grid-cols-1 gap-3 xl:grid-cols-2">
                  <AnswerPane
                    title="AI 當初回答（常見版本）"
                    content={item.aiOriginalAnswer || "（目前無可用原始回答）"}
                    tone="old"
                  />
                  <AnswerPane
                    title="建議修改答案"
                    content={item.proposedAnswer || "（目前沒有可用建議答案）"}
                    tone="new"
                  />
                </div>
                ...
                <div className="mt-3 rounded-lg border border-[var(--theme-border)] bg-[var(--theme-bg-card)] p-3 text-sm text-[var(--theme-text-primary)] whitespace-pre-wrap">
                  AI 補跑答案：{"\n"}
                  {item.aiRerunAnswer || "（尚未取得）"}
                </div>

                <div className="mt-3 rounded-lg border border-[var(--theme-border)] bg-[var(--theme-bg-card)] p-3">
                  <div className="mb-2 text-xs text-[var(--theme-text-secondary)]">群眾回饋可視化（Top 3）</div>
                  {feedbackList.length === 0 ? (
                    <p className="text-sm text-[var(--theme-text-secondary)]">目前沒有可視化的 feedback 資料。</p>
                  ) : (
...
function AnswerPane({ title, content, tone }: { title: string; content: string; tone: "old" | "new" }) {
  const toneClass = tone === "old" ? "border-amber-300 bg-amber-50" : "border-emerald-300 bg-emerald-50"

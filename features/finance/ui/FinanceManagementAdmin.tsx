...
// --- Mock Data for Expenses (Matches your screenshots) ---
...
// 1. Income Modal (With Fee Calculator)
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[200] flex items-center justify-center p-4">
      <div className="bg-[var(--theme-bg-card)] rounded-2xl w-full max-w-md max-h-[90vh] overflow-y-auto border border-[var(--theme-border)]">
        ...
        <div className="p-4 border-t border-[var(--theme-border)] flex gap-3">
          <button onClick={onClose} className="flex-1 px-4 py-3 rounded-xl font-semibold border border-[var(--theme-border)] text-[var(--theme-text-secondary)] hover:bg-[var(--theme-accent-light)]">
            取消
          </button>
          <button onClick={onSave} className="flex-1 px-4 py-3 rounded-xl font-semibold bg-[var(--theme-accent)] text-[var(--theme-bg-primary)] hover:opacity-90">
            {isEditing ? "儲存" : "新增"}
          </button>
        </div>
      </div>
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[200] flex items-center justify-center p-4">
      <div className="bg-[var(--theme-bg-card)] rounded-2xl w-full max-w-md max-h-[90vh] overflow-y-auto border border-[var(--theme-border)]">
        ...
        <div className="p-4 border-t border-[var(--theme-border)] flex gap-3">
          ...
          <button onClick={onSave} className="flex-1 px-4 py-3 rounded-xl font-semibold bg-red-500 text-white hover:bg-red-600 transition-colors">
            {isEditing ? "儲存" : "新增支出"}
          </button>
        </div>
      </div>
...
// --- Main Component ---
// This is what gets rendered when you click "管理費/收支" in the sidebar
export function FinanceManagementAdmin() {
  ...
  const [incomeFormData, setIncomeFormData] = useState<FinanceRecord>({
    room: "", amount: 0, due: "", invoice: "", paid: false,
  })
  ...
  const [expenseFormData, setExpenseFormData] = useState<ExpenseRecord>({
    id: "", date: new Date().toISOString().split("T")[0], item: "", category: "維護費", amount: 0, vendor: ""
  })

  // --- Report Calculations (Auto-updates graphs based on data) ---
  const reportData = useMemo(() => {
    // 1. Total Income (Sum of all PAID income records)
    const totalIncome = records.filter(r => r.paid).reduce((sum, r) => sum + (r.amount || 0), 0)

    // 2. Total Expense (Sum of all expenses)
    ...
    return {
      ...
      categoryAnalysis: Object.entries(categoryTotals).map(([name, value]) => ({
        name,
        value,
  ...
  const handleSaveExpense = () => {
    if (expenseEditingId) {
      setExpenses(prev => prev.map(e => e.id === expenseEditingId ? { ...expenseFormData, id: expenseEditingId } : e))
    } else {
    ...
    <div className="bg-[var(--theme-bg-card)] border border-[var(--theme-border)] rounded-2xl p-5 min-h-[600px]">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        ...
        <div className="flex bg-[var(--theme-bg-secondary)] p-1 rounded-lg border border-[var(--theme-border)]">
          ...
          >
             <span className="material-icons text-sm">payments</span> 收費 (Income)
          </button>
          <button
          ...
          >
             <span className="material-icons text-sm">shopping_cart</span> 支出 (Expenditure)
          </button>
          <button
          ...
          >
             <span className="material-icons text-sm">analytics</span> 報表 (Reports)
          </button>
        </div>
      </div>

      {/* === 1. INCOME TAB (Your existing table + Calculator) === */}
      {activeTab === "income" && (
            ...
            <table className="w-full border-collapse">
              <thead>
                <tr className="bg-[var(--theme-accent-light)]">
                  <th className="p-3 text-left text-[var(--theme-accent)] border-b border-[var(--theme-border)] rounded-tl-lg">房號</th>
                  ...
                  <th className="p-3 text-left text-[var(--theme-accent)] border-b border-[var(--theme-border)] rounded-tr-lg">操作</th>
                </tr>
              </thead>
              <tbody>
                {records.length > 0 ? (
                  records.map((row, index) => (
                    <tr key={row.id || `new-${index}`} className="hover:bg-[var(--theme-bg-secondary)] transition-colors border-b border-[var(--theme-border)] last:border-0">
                      <td className="p-3 text-[var(--theme-text-primary)]">{row.room || "-"}</td>
                      <td className="p-3 text-[var(--theme-text-primary)] font-medium">${row.amount?.toLocaleString() || 0}</td>
                      <td className="p-3 text-[var(--theme-text-secondary)]">{row.due ? new Date(row.due).toLocaleDateString("zh-TW") : "-"}</td>
                      <td className="p-3 text-[var(--theme-text-secondary)]">{row.invoice || "-"}</td>
                      <td className="p-3">
                        <span className={`px-2 py-1 rounded-full text-xs font-bold ${row.paid ? "bg-green-500/20 text-green-500" : "bg-red-500/20 text-red-500"}`}>
                          {row.paid ? "已繳" : "未繳"}
                        </span>
                      </td>
                      <td className="p-3">
                        <div className="flex gap-2">
                          <button onClick={() => handleEditIncome(index)} className="p-2 rounded-lg text-[var(--theme-text-secondary)] hover:bg-[var(--theme-accent-light)] hover:text-[var(--theme-accent)] transition-all">
                            <span className="material-icons text-lg">edit</span>
                          </button>
                          {row.id && (
                            <button onClick={() => removeRecord(row.id!)} className="p-2 rounded-lg text-[var(--theme-text-secondary)] hover:bg-red-500/10 hover:text-red-500 transition-all">
                  ...
                  <tr><td colSpan={6} className="p-8 text-center text-[var(--theme-text-secondary)]">無記錄</td></tr>
                )}
              </tbody>
            </table>
          </div>
      ...
      {/* === 2. EXPENSE TAB (Matches your "Expenditure" Screenshot) === */}
      {activeTab === "expense" && (
        <div className="animate-fadeIn">
          <div className="flex justify-end mb-4">
            <button
          ...
          <div className="overflow-x-auto">
            <table className="w-full border-collapse">
              ...
              <tbody>
                {expenses.length > 0 ? (
                  expenses.map((exp) => (
                    <tr key={exp.id} className="hover:bg-[var(--theme-bg-secondary)] transition-colors border-b border-[var(--theme-border)] last:border-0">
                      ...
                      <td className="p-3 text-[var(--theme-text-secondary)]"><span className="bg-[var(--theme-bg-secondary)] px-2 py-1 rounded text-xs">{exp.category}</span></td>
                      ...
                      <td className="p-3">
                        <div className="flex gap-2">
                          <button onClick={() => handleEditExpense(exp.id)} className="p-2 rounded-lg text-[var(--theme-text-secondary)] hover:bg-[var(--theme-accent-light)] hover:text-[var(--theme-accent)] transition-all">
                            <span className="material-icons text-lg">edit</span>
                          </button>
                          <button onClick={() => handleDeleteExpense(exp.id)} className="p-2 rounded-lg text-[var(--theme-text-secondary)] hover:bg-red-500/10 hover:text-red-500 transition-all">
                            <span className="material-icons text-lg">delete</span>
                          </button>
                        </div>
                      </td>
                  ...
                  <tr><td colSpan={6} className="p-8 text-center text-[var(--theme-text-secondary)]">無支出記錄</td></tr>
                )}
              </tbody>
            </table>
          </div>
      ...
      {/* === 3. REPORTS TAB (Matches your "Reports" Screenshot) === */}
      {activeTab === "report" && (
        <div className="animate-fadeIn space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="p-6 rounded-xl border border-[var(--theme-border)] bg-[var(--theme-bg-secondary)] relative overflow-hidden group">
              ...
              <div className="text-[var(--theme-text-primary)] font-bold mb-1">總收入 (Total Income)</div>
            ...
            <div className="p-6 rounded-xl border border-[var(--theme-border)] bg-[var(--theme-bg-secondary)] relative overflow-hidden group">
              ...
              <div className="text-red-400 font-bold mb-1">總支出 (Total Expenses)</div>
              <div className="text-3xl font-bold text-white mb-2 tracking-tight">
            ...
            <div className="p-6 rounded-xl border border-[var(--theme-border)] bg-[var(--theme-bg-secondary)] relative overflow-hidden group">
              ...
              <div className="text-[var(--theme-text-primary)] font-bold mb-1">本期損益 (Net Income)</div>
              <div className={`text-3xl font-bold mb-2 tracking-tight ${reportData.netIncome >= 0 ? 'text-green-500' : 'text-white'}`}>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            {/* Right: Assets Overview (Mock Data) */}
            <div className="p-5 rounded-xl border border-[var(--theme-border)] bg-[var(--theme-bg-secondary)]">
              <h3 className="flex items-center gap-2 text-[var(--theme-text-primary)] font-bold mb-6">

...
async function findLineUserByUnit(supabase: any, unitId: string) {
  let lineUserId = null
  ...
  try {
    ...
    if (directProfiles && directProfiles.length > 0) {
      const profileWithLine = directProfiles.find((p: any) => p.line_user_id)
      if (profileWithLine) {
        return {
      ...
      const profileIds = directProfiles.map((p: any) => p.id).filter(Boolean)
      if (profileIds.length > 0) {
        ...
        const lineUser = (lineUsers || []).find((u: any) => u.line_user_id)
        if (lineUser) {
          return {
  ...
  try {
    ...
    if (profileIds.length > 0) {
      ...
      const profileWithLine = (profiles || []).find((p: any) => p.line_user_id)
      if (profileWithLine) {
        return {
          lineUserId: profileWithLine.line_user_id,
          lineDisplayName: profileWithLine.line_display_name || profileWithLine.name,

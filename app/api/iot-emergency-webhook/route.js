...
// --- 建立 LINE 推播目標（BOT2 優先，BOT1 備用，自動去重） ---
...
const pushTargets = [
  ...
  if (!target.client || !target.token) return false;
...
async function resolveRecipient(record) {
  ...
  const explicitLineUserId =
    eventData.line_user_id ||
    eventData.notify_line_user_id ||
    eventData.recipient_line_user_id;
  if (explicitLineUserId) {
    return { lineUserId: explicitLineUserId, profile: null };
  }
  ...
  try {
    ...
    if (record.emergency_contact_id || (record.linked_record_id && record.linked_record_type)) {
      try {
        const contactId =
          ...
          (record.linked_record_type === 'emergency' ? record.linked_record_id : null);
    ...
    if (successTargets.length === 0) {
      return Response.json(
        { success: false, message: 'LINE push failed for all configured bots' },
        { status: 502 }
      );
    }
